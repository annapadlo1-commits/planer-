  ...
  return <section className={`solver-workspace ${published ? "published" : ""}`}>
    ...
    {variantDiagnostics&&<><button className="drawer-scrim top" onClick={()=>setVariantDiagnostics(null)}/><aside className="drawer role-drawer top candidate-diagnostics-drawer">
      ...
      <div className="drawer-content">
        ...
        {variantDiagnostics.summary.eligible>0&&<div className="solver-v2-notice warning"><AlertTriangle/><span><strong>{variantDiagnostics.summary.eligible} osób nie ma indywidualnej twardej blokady</strong><small>{variantDiagnostics.publishedScheduleId?"To nie gwarantuje, że dopisanie zachowa poprawność całego miesiąca. Przycisk wykona ponowną kontrolę globalną przed zapisem.":"Solver mógł wykorzystać te osoby w innym miejscu albo zachować je jako wymaganą rezerwę. Ręczna korekta jest dostępna dopiero w opublikowanym Grafiku operacyjnym."}</small></span></div>}
