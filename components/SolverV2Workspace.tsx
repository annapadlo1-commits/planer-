...
import { assignmentStyle, availabilityLabel, dateLabel, dutyStyle, locationStyle, money, monthLabel, monthWeeks, preferenceLevelLabel, publicationStatus, reasonLabel, roleStyle, shortDayLabel, timeLabel, timestampLabel, workloadHours, workloadReason, workloadReasonCode } from "@/lib/solver-v2-workspace-presentation";
export { stablePaletteIndex } from "@/lib/solver-v2-workspace-presentation";
...
export function SolverV2Workspace({ workspace, baselineWorkspace=null, timezone, published = false, operational=false, onOperationalChanged, notify, fail, leaderEditable=false, onLeaderChanged, onLeaderBusyChange, onOpenAdHoc, initialView="CALENDAR" }: Props) {
  ...
  const scheduleRoles=workspaceRoles;
  ...
  return <section className={`solver-workspace ${published ? "published" : ""} ${leaderEditable?"leader-studio":""}`} onPointerDownCapture={event=>{lastLeaderPointerType.current=event.pointerType;}} onKeyDownCapture={()=>{lastLeaderPointerType.current="";}}>
    ...
    {(!leaderEditable||!leaderContext)&&<div className="solver-global-filters">
      ...
      {leaderEditable&&<section className="leader-employee-pool" aria-label="Pracownicy do ręcznego ułożenia">
        <header><Users/><span><strong>Pracownicy do ułożenia</strong><small>Desktop: złap osobę i upuść ją na dokładną zmianę. Telefon: dotknij osoby, potem zmiany — albo najpierw zmiany, potem osoby.</small></span><button type="button" className={`secondary-button${mobileLeaderEmployeeId?"":" placeholder"}`} disabled={leaderBusy||!mobileLeaderEmployeeId} aria-hidden={!mobileLeaderEmployeeId} tabIndex={mobileLeaderEmployeeId?0:-1} onClick={()=>{setMobileLeaderEmployeeId("");setLeaderEmployeeId("");}}>Wyczyść wybór osoby</button></header>
    ...
    {workspaceView==="CALENDAR"&&<section className="solver-weekly-workspace">
      ...
      {dragPreview&&<div className={`solver-v2-notice ${dragPreview.loading?"info":dragPreview.valid?"success":"danger"}`} role="status">{dragPreview.loading?<RefreshCw className="spin"/>:dragPreview.valid?<Check/>:<AlertTriangle/>}<span><strong>{dragPreview.loading?"Kontrola przed upuszczeniem":dragPreview.valid?"Szkic zaktualizowany":"Nie można upuścić tutaj"}</strong><small>{dragPreview.message}</small></span></div>}
    ...
    {employeeDetail&&<><button className="drawer-scrim top" onClick={()=>setEmployeeDetailId("")}/><aside className="drawer top solver-employee-compare-drawer">
      ...
      <div className="drawer-content">
        ...
        <div className="employee-compare-toolbar"><label><ArrowLeftRight/> Porównaj z<select value={comparisonEmployeeId} onChange={event=>setComparisonEmployeeId(event.target.value)}><option value="">Nie porównuj</option>{scheduleEmployees.filter(employee=>employee.id!==employeeDetail.id).map(employee=><option value={employee.id} key={employee.id}>{employee.lastName} {employee.firstName} • {employee.employeeNo}</option>)}</select></label><small>Dostępność, zmiany, obowiązki i lokale obu osób są warstwami tego samego dnia. Każda korekta przechodzi końcową kontrolę serwera.</small></div>
        <div className="employee-compare-legend"><span className="primary-person">{employeeDetailName}</span>{comparisonEmployee&&<span className="comparison-person">{comparisonEmployee.firstName} {comparisonEmployee.lastName}</span>}<small>{comparisonAvailabilityLoading?"Sprawdzamy deklaracje dostępności…":"Czerwony status oznacza niedostępność, a „Wolne w grafiku” wyłącznie brak zmiany."}</small></div>
        ...
        <section className="employee-unified-month-calendar" aria-label="Wspólny kalendarz dostępności i zmian porównywanych pracowników">
          <div className="employee-unified-weekdays" aria-hidden="true">{["Pon.","Wt.","Śr.","Czw.","Pt.","Sob.","Niedz."].map(day=><span key={day}>{day}</span>)}</div>
          <div className="employee-unified-weeks">{weeks.map((week,weekIndex)=><section key={week[0]} aria-label={`Tydzień ${weekIndex+1}`}>{week.map(date=>{
            if(date.slice(0,7)!==workspace.context.month.slice(0,7))return <article className="employee-unified-day outside-month" aria-hidden="true" key={date}/>;
            const primary=employeeEntries(employeeDetail.id,date);
            const compared=comparisonEmployee?employeeEntries(comparisonEmployee.id,date):[];
            const people=[{employee:employeeDetail,name:employeeDetailShortName,entries:primary,kind:"primary" as const},...(comparisonEmployee?[{employee:comparisonEmployee,name:comparisonEmployee.firstName,entries:compared,kind:"comparison" as const}]:[])];
            return <article className="employee-unified-day" key={date}>
              <header><CalendarDays/><strong>{shortDayLabel(date)}</strong></header>
              <div className="employee-unified-people">{people.map(person=>{const availability=comparisonAvailability.find(item=>item.employeeId===person.employee.id&&item.date===date);return <section className={`employee-unified-person ${person.kind}`} key={person.employee.id}>
                <header><strong>{person.name}</strong><span className={`availability-${(availability?.status??"unknown").toLowerCase()}`}>{availability?.label??(comparisonAvailabilityLoading?"Sprawdzamy…":"Brak deklaracji")}</span></header>
                {person.entries.map(({shift,assignment})=><div className="employee-unified-assignment" style={assignmentStyle(assignment.role.id,shift.location.id)} key={assignment.id}><span><b>{timeLabel(shift.startsAt,shift.location.timezone??timezone)}–{timeLabel(shift.endsAt,shift.location.timezone??timezone)}</b><small>{shift.location.name} • {assignment.role.name}</small></span>{assignment.duties.length>0&&<span className="solver-week-duties">{assignment.duties.map(duty=><em style={dutyStyle(duty.id)} key={duty.id}>{duty.name}</em>)}</span>}{leaderEditable&&!comparisonEmployee&&<button type="button" className="possible-swap-day" onClick={()=>void openLeaderEdit({assignmentId:assignment.id})}><ArrowLeftRight/> Możliwa zamiana</button>}{leaderEditable&&comparisonEmployee&&<button aria-label="Edytuj przydział" onClick={()=>void openLeaderEdit({assignmentId:assignment.id})}><Edit3/></button>}</div>)}
                {!person.entries.length&&<p>Wolne w grafiku</p>}
              </section>})}</div>
              {comparisonEmployee&&((primary.length&&!compared.length)||(!primary.length&&compared.length))&&<button type="button" className="swap-opportunity" disabled={!leaderEditable} onClick={()=>void openLeaderEdit({assignmentId:(primary[0]??compared[0]).assignment.id,preferredEmployeeId:primary.length?comparisonEmployee.id:employeeDetail.id})}><ArrowLeftRight/> Sprawdź, czy {primary.length?comparisonEmployee.firstName:employeeDetailShortName} może przejąć tę zmianę. System najpierw kontroluje rolę, lokal i obowiązek, a przy zapisie cały miesiąc.</button>}
            </article>;
          })}</section>)}</div>
        </section>
      </div>
