...
import {
  ...
  selectRecoveryCandidate, proposeRecoveryIncidentRate, decideRecoveryIncidentRate,
...
const incidentStatusLabels:Record<string,string>={
  DRAFT:"Wersja robocza",OPEN:"Aktywny",PREPARED:"Plan naprawy przygotowany",
  OFFERS_SENT:"Oferty wysłane",APPLIED:"Zastosowany",RESOLVED:"Zakończony",CANCELLED:"Anulowany",
};

const actionRiskLabels: Record<string, string> = {
  LOW: "Niskie ryzyko", MEDIUM: "Średnie ryzyko",
  HIGH: "Wysokie ryzyko", CRITICAL: "Krytyczne ryzyko",
};

const incidentRateStatusLabels: Record<string, string> = {
  PROPOSED: "Proponowana", APPROVED: "Zatwierdzona",
  REJECTED: "Odrzucona", SUPERSEDED: "Zastąpiona",
};

function enumLabel(labels: Record<string, string>, value: unknown, fallback: string) {
...
function riskClass(value: unknown) {
  const code = typeof value === "string" ? value.trim().toUpperCase() : "";
  return actionRiskLabels[code] ? code.toLowerCase() : "unknown";
}
...
export function RecoveryCenter({ month, employees = [], currency = "PLN", employeeMode = false, notify, fail, reload, initialTab="SHORTAGES", focusRoleId=null, focusDate=null, onCreateFullEmployee }: Props) {
  ...
  const [rateForm, setRateForm] = useState({ employeeId: "", rate: "", reason: "" });
  ...
  const proposeIncidentRate = async () => {
    if (!detail || !rateForm.employeeId || !rateForm.rate || rateForm.reason.trim().length < 5) return;
    setBusy(true);
    try {
      await proposeRecoveryIncidentRate(supabase, { incidentId: detail.id, employeeId: rateForm.employeeId, rateMinor: Math.round(Number(rateForm.rate) * 100), currency, validFrom: detail.startsOn, validTo: detail.endsOn, reason: rateForm.reason });
      setRateForm({ employeeId: "", rate: "", reason: "" });
      setDetail(await getRecoveryIncidentDetail(supabase, detail.id));
      notify("Stawkę zapisano jako propozycję. Nie wpływa na solver ani koszt do czasu zatwierdzenia przez właściciela.");
    } catch (error) { fail(recoveryErrorMessage(error)); }
    finally { setBusy(false); }
  };

  const decideIncidentRate = async (rateId: string, approve: boolean, proposedRateMinor: number) => {
    if (!detail) return;
    const reason = approve ? "Zatwierdzono do rozliczenia incydentu" : "Odrzucono propozycję stawki";
    setBusy(true);
    try {
      await decideRecoveryIncidentRate(supabase, { rateId, approve, approvedRateMinor: approve ? proposedRateMinor : null, reason });
      setDetail(await getRecoveryIncidentDetail(supabase, detail.id));
      notify(approve ? "Stawka zatwierdzona. Od tej chwili snapshot i solver uwzględniają ją w zakresie dat incydentu." : "Propozycja stawki odrzucona i nie wpłynie na koszt.");
    } catch (error) { fail(recoveryErrorMessage(error)); }
    finally { setBusy(false); }
  };
  ...
  const incidentAdHoc = detail ? workspace.adHocPool.filter(worker =>
    worker.active
    && (!worker.availableFrom || worker.availableFrom <= detail.endsOn)
    && (!worker.availableTo || worker.availableTo >= detail.startsOn)
    && (!incidentRoleNames.size || incidentRoleNames.has(worker.roleName))
  ) : [];
  ...
  return <section className="recovery-center">
    ...
    {activeTab === "INCIDENTS" && <div className="recovery-incident-list">
      {workspace.incidents.map(incident => <button key={incident.id} style={roleStyle(incident.roleColor)} onClick={async () => setDetail(await getRecoveryIncidentDetail(supabase, incident.id))}>
        <i/><span><small>{incidentLabels[incident.type] ?? "Inny incydent"} • {incidentStatusLabels[incident.status]??"Status wymaga sprawdzenia"}</small><strong>{incident.title}</strong><em>{incident.startsOn} – {incident.endsOn}{incident.employeeName ? ` • ${incident.employeeName}` : ""}</em></span>
