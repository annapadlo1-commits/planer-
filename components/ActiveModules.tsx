...
function OperationalCalendarPanel({ context, month, busy, save, review }: {
  ...
  const selectedWeekdays=useMemo(()=>{const result=new Set<number>();if(!startDate||!endDate)return result;for(const cursor=new Date(`${startDate}T12:00:00Z`),last=new Date(`${endDate}T12:00:00Z`);cursor<=last;cursor.setUTCDate(cursor.getUTCDate()+1))result.add(((cursor.getUTCDay()+6)%7)+1);return result;},[startDate,endDate]);
  const visibleTemplates = useMemo(() => templates.filter(template => template.locationId === locationId && template.dayMask.some(day=>selectedWeekdays.has(day))), [locationId, selectedWeekdays, templates]);
