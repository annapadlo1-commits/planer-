...
function OperationalCalendarPanel({ context, month, busy, save, review }: {
  ...
  const filteredAvailabilitySummary=(context.availabilitySummary??[]).filter(item=>(!availabilityRoleIds.length||availabilityRoleIds.includes(item.roleId))&&item.date>=availabilityFrom&&item.date<=availabilityTo&&(availabilityRisk==="ALL"||(availabilityRisk==="PENDING"?item.pendingCount>0:(item.availableCount??0)<(item.totalCount??0)))&&(!availabilityQuery.trim()||`${item.roleName} ${item.date} ${item.hardEmployees.join(" ")} ${item.softEmployees.join(" ")} ${item.pendingEmployees.join(" ")}`.toLocaleLowerCase("pl-PL").includes(availabilityQuery.trim().toLocaleLowerCase("pl-PL"))));
