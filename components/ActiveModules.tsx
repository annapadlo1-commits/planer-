...
type AvailabilitySummaryItem = { date:string; roleId:string; roleName:string; categoryId?:string; categoryName?:string; locationId?:string; locationName?:string; totalCount?:number; availableCount?:number; recordedCount?:number; progressPercent?:number; lastUpdatedAt?:string|null; hardCount:number; softCount:number; pendingCount:number; hardEmployees:string[]; softEmployees:string[]; pendingEmployees:string[] };
type WorkforceCalendarContext = {
  ...
  shiftTemplates?: { id: string; name: string; code: string; locationId: string; startsAt: string; endsAt: string; endsNextDay: boolean; dayMask: number[]; roleIds: string[]; roleNames: string[] }[];
  ...
  availabilityScopedSummary?: AvailabilitySummaryItem[];
};
...
export type EmployeePortalSection = "overview" | "my-schedule" | "company-schedule" | "swaps";
...
export function ActiveModules({
  ...
  const loadOperationalContext = useCallback(async () => {
    ...
    const result = await supabase.rpc("workforce_calendar_context_uat_v4", { p_month: selectedMonthDate });
    if (result.error) {
  ...
  async function saveTimeConstraint(entry: { dates: string[]; kind: "AVAILABLE" | "PREFER_NOT_TO_WORK" | "CANNOT_WORK" | "LEAVE" | "SICKNESS"; allDay: boolean; start?: string; end?: string; preferredLocationId?: string; note: string }) {
    let publicationConflicts: Array<{ scheduleName?: string; publishedAt?: string; date?: string; shiftName?: string; locationName?: string }> = [];
    const absenceRequest=["CANNOT_WORK","LEAVE","SICKNESS"].includes(entry.kind);
    if (absenceRequest && supabase) {
    ...
    if(uatMasterEmployeeId&&["LEAVE","SICKNESS"].includes(entry.kind)){
      fail("Wniosku urlopowego ani L4 nie można złożyć w symulowanym trybie MASTER. Zaloguj się na rzeczywiste konto pracownika.");
      return false;
    }
    ...
    const result = await rpc(requestMode
      ...
      p_dates: entry.dates,
      ...(requestMode?{p_request_type:requestType}:{p_kind:entry.kind}),
      p_all_day: entry.allDay,
      p_local_start: entry.allDay ? null : entry.start,
      p_local_end: entry.allDay ? null : entry.end,
      ...(!requestMode?{p_preferred_location_id:entry.preferredLocationId || null}:{}),
      p_note: entry.note || null,
    });
    if (!result) return false;
    const reviewCount = Number((result as { pendingReviewDays?: number;pending?:number }).pendingReviewDays ?? (result as {pending?:number}).pending ?? 0);
    notify(publicationConflicts.length > 0
      ...
      : entry.kind==="LEAVE" ? "Wniosek urlopowy trafił do skrzynki lidera. Do czasu decyzji ma status oczekujący."
      : entry.kind==="SICKNESS" ? "L4 zapisano od razu. Lider dostał zadanie potwierdzenia odbioru i organizacji zastępstwa."
      : reviewCount > 0
...
function OperationalCalendarPanel({ context, month, busy, save, review }: {
  ...
  const [availabilityCategoryIds,setAvailabilityCategoryIds]=useState<string[]>([]);
  const [availabilityRoleIds,setAvailabilityRoleIds]=useState<string[]>([]);
  const [availabilityLocationIds,setAvailabilityLocationIds]=useState<string[]>([]);
  const [availabilityFrom,setAvailabilityFrom]=useState(`${month}-01`);
  const [availabilityTo,setAvailabilityTo]=useState(`${month}-${String(new Date(Number(month.slice(0,4)),Number(month.slice(5,7)),0).getDate()).padStart(2,"0")}`);
  const [availabilityRisk,setAvailabilityRisk]=useState<"ALL"|"SHORTAGE"|"PENDING">("ALL");
  ...
  const selectedDates=useMemo(()=>{const result:string[]=[];if(!startDate||!endDate)return result;for(const cursor=new Date(`${startDate}T12:00:00Z`),last=new Date(`${endDate}T12:00:00Z`);cursor<=last;cursor.setUTCDate(cursor.getUTCDate()+1))result.push(cursor.toISOString().slice(0,10));return result;},[startDate,endDate]);
  const templateOccurrences=useMemo(()=>new Map(templates.map(template=>[template.id,selectedDates.filter(date=>template.dayMask.includes(((new Date(`${date}T12:00:00Z`).getUTCDay()+6)%7)+1))])),[selectedDates,templates]);
  const visibleTemplates = useMemo(() => templates.filter(template => template.locationId === locationId && template.roleIds.includes(roleId) && (templateOccurrences.get(template.id)?.length??0)>0), [locationId, roleId, templateOccurrences, templates]);
  const availabilityRows=context.availabilityScopedSummary?.length?context.availabilityScopedSummary:(context.availabilitySummary??[]);
  const availabilityCategories=Array.from(new Map(availabilityRows.filter(item=>item.categoryId).map(item=>[item.categoryId!,item.categoryName??"Kategoria"])).entries());
  const availabilityLocations=Array.from(new Map(availabilityRows.filter(item=>item.locationId).map(item=>[item.locationId!,item.locationName??"Lokal"])).entries());
  const filteredAvailabilitySummary=availabilityRows.filter(item=>(!availabilityCategoryIds.length||Boolean(item.categoryId&&availabilityCategoryIds.includes(item.categoryId)))&&(!availabilityRoleIds.length||availabilityRoleIds.includes(item.roleId))&&(!availabilityLocationIds.length||Boolean(item.locationId&&availabilityLocationIds.includes(item.locationId)))&&item.date>=availabilityFrom&&item.date<=availabilityTo&&(availabilityRisk==="ALL"||(availabilityRisk==="PENDING"?item.pendingCount>0:(item.availableCount??0)<(item.totalCount??0)))&&(!availabilityQuery.trim()||`${item.categoryName??""} ${item.roleName} ${item.locationName??""} ${item.date} ${item.hardEmployees.join(" ")} ${item.softEmployees.join(" ")} ${item.pendingEmployees.join(" ")}`.toLocaleLowerCase("pl-PL").includes(availabilityQuery.trim().toLocaleLowerCase("pl-PL"))));
  useEffect(() => { setSelectedTemplates(visibleTemplates.map(template => template.id)); }, [locationId, startDate, endDate, roleId]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  return <section className="operational-calendar-panel">
    ...
    {context.canManage && <details className="operational-additional-tools">
      ...
      {kind === "EVENT" ? <><fieldset className="operational-card-picker"><legend>Lokal</legend>{locations.map(location => <button type="button" key={location.id} className={locationId === location.id ? "active" : ""} onClick={() => setLocationId(location.id)}>{location.name}</button>)}</fieldset><fieldset className="operational-shift-picker"><legend>Zmiany aktywne w wybranym okresie dla roli {roles.find(role=>role.id===roleId)?.name??"—"}</legend>{visibleTemplates.map(template => {const occurrences=templateOccurrences.get(template.id)??[];return <label key={template.id}><input type="checkbox" checked={selectedTemplates.includes(template.id)} onChange={() => setSelectedTemplates(current => current.includes(template.id) ? current.filter(id => id !== template.id) : [...current, template.id])} /><span><b>{roles.find(role=>role.id===roleId)?.name} • {locations.find(location=>location.id===locationId)?.name}</b><small>{template.name} • {String(template.startsAt).slice(0, 5)}–{String(template.endsAt).slice(0, 5)} • {occurrences.length} {occurrences.length===1?"wystąpienie":"wystąpienia"}: {occurrences.join(", ")}</small></span></label>;})}</fieldset>{!visibleTemplates.length&&<div className="solver-v2-notice warning"><AlertTriangle/><span><strong>Brak zmian tej roli w wybranych dniach</strong><small>Zmień datę, lokal albo rolę. Lista pokazuje wyłącznie zmiany, które mają zapotrzebowanie na wybraną rolę i fizycznie występują w co najmniej jednym dniu zakresu.</small></span></div>}<label className="compact-number">Dodatkowe osoby na każdej zaznaczonej zmianie<input type="number" min={1} max={500} value={count} onChange={event => setCount(Number(event.target.value))} /></label></> : <label className="compact-number">Ile twardych niedostępności roli można przyjąć automatycznie?<input type="number" min={0} max={500} value={limit} onChange={event => setLimit(Number(event.target.value))} /><small>Kolejne zgłoszenie będzie oczekiwać na decyzję lidera.</small></label>}
    ...
    {context.canManage && Boolean(availabilityRows.length) && <details className="availability-daily-summary technical-details"><summary><span><CalendarDays /><strong>Techniczne zestawienie dostępności dzień po dniu</strong><small>Filtruj po kategorii, wielu rolach, lokalach, zakresie dat i poziomie ryzyka. Filtry pozostają aktywne po decyzjach i zapisach.</small></span></summary><div className="availability-summary-workspace"><div className="matrix-v2-filterbar"><fieldset><legend>Kategorie</legend>{availabilityCategories.map(([id,name])=><label className="check-label" key={id}><input type="checkbox" checked={availabilityCategoryIds.includes(id)} onChange={event=>setAvailabilityCategoryIds(event.target.checked?[...availabilityCategoryIds,id]:availabilityCategoryIds.filter(value=>value!==id))}/>{name}</label>)}</fieldset><CheckboxDropdown label="Role" selectedCount={availabilityRoleIds.length} totalCount={roles.length}>{roles.map(role=><label className="check-label" key={role.id}><input type="checkbox" checked={availabilityRoleIds.includes(role.id)} onChange={event=>setAvailabilityRoleIds(event.target.checked?[...availabilityRoleIds,role.id]:availabilityRoleIds.filter(id=>id!==role.id))}/>{role.name}</label>)}</CheckboxDropdown><fieldset><legend>Lokale</legend>{availabilityLocations.map(([id,name])=><label className="check-label" key={id}><input type="checkbox" checked={availabilityLocationIds.includes(id)} onChange={event=>setAvailabilityLocationIds(event.target.checked?[...availabilityLocationIds,id]:availabilityLocationIds.filter(value=>value!==id))}/>{name}</label>)}</fieldset><label>Od<input type="date" value={availabilityFrom} onChange={event=>setAvailabilityFrom(event.target.value)}/></label><label>Do<input type="date" value={availabilityTo} onChange={event=>setAvailabilityTo(event.target.value)}/></label><label>Ryzyko<select value={availabilityRisk} onChange={event=>setAvailabilityRisk(event.target.value as "ALL"|"SHORTAGE"|"PENDING")}><option value="ALL">Wszystkie</option><option value="SHORTAGE">Niepełna dostępność</option><option value="PENDING">Oczekujące decyzje</option></select></label><label>Szukaj<input value={availabilityQuery} onChange={event=>setAvailabilityQuery(event.target.value)} placeholder="Kategoria, rola, lokal, osoba lub data"/></label><button type="button" className="secondary-button" onClick={()=>{setAvailabilityCategoryIds([]);setAvailabilityRoleIds([]);setAvailabilityLocationIds([]);setAvailabilityFrom(`${month}-01`);setAvailabilityTo(`${month}-${String(new Date(Number(month.slice(0,4)),Number(month.slice(5,7)),0).getDate()).padStart(2,"0")}`);setAvailabilityRisk("ALL");setAvailabilityQuery("");}}>Wyczyść filtry</button></div><section className="availability-risk-summary"><strong>Ryzyko dostępności: {filteredAvailabilitySummary.filter(item=>(item.availableCount??0)<(item.totalCount??0)).length} zakresów dzień/rola/lokal wymaga uwagi</strong><small>Najpierw pokazujemy zakresy z najmniejszym udziałem dostępnych osób; szczegóły wskazują liczbę, procent i konkretne zgłoszenia.</small></section><div>{[...filteredAvailabilitySummary].sort((a,b)=>((a.availableCount??0)/Math.max(1,a.totalCount??1))-((b.availableCount??0)/Math.max(1,b.totalCount??1))).map(item=><details key={`${item.date}:${item.roleId}:${item.locationId??"all"}`}><summary><b>{item.date} • {item.categoryName?`${item.categoryName} • `:""}{item.roleName}{item.locationName?` • ${item.locationName}`:""}</b><span><em>{item.hardCount} twardych</em><em>{item.availableCount??"—"}/{item.totalCount??"—"} dostępnych</em><em>{item.totalCount?Math.round(((item.totalCount-(item.availableCount??0))/item.totalCount)*100):0}% niedostępnych</em><em>{item.progressPercent??0}% zgłoszeń</em>{item.pendingCount>0&&<em>{item.pendingCount} oczekuje</em>}</span></summary><p>{item.hardEmployees.length?`Twardo niedostępni: ${item.hardEmployees.join(", ")}. `:""}{item.softEmployees.length?`Wolą nie pracować: ${item.softEmployees.join(", ")}. `:""}{item.pendingEmployees.length?`Do decyzji: ${item.pendingEmployees.join(", ")}.`:""}</p>{item.lastUpdatedAt&&<small>Ostatnia aktualizacja: {new Intl.DateTimeFormat("pl-PL",{dateStyle:"short",timeStyle:"short"}).format(new Date(item.lastUpdatedAt))}</small>}</details>)}</div>{!filteredAvailabilitySummary.length&&<p>Brak pozycji spełniających aktywne filtry.</p>}</div></details>}
...
function EmployeePortal({ portal, month, timezone, dynamic, roleNames, roleColors, openAvailability, openPreferences, requestSwap, loadSwapCandidates, decideSwapAsEmployee, decideSwapAsLeader, masterMode, busy, section, availabilityWorkspace, locations, saveAvailability, fail, openEmployeeSection }: {
  ...
  const sectionCopy: Record<EmployeePortalSection, { title: string; subtitle: string }> = {
    ...
    "my-schedule": { title: "Mój grafik", subtitle: "Zmiany, dostępność, preferencje, urlopy, L4 i twarde nieobecności w jednym kalendarzu." },
...
function CompanyScheduleCalendar({ calendar, month, timezone, events, roleColors }: { calendar: CompanyCalendar; month: string; timezone: string; events: WorkforceCalendarEvent[]; roleColors: Record<string,string> }) {
  ...
  const cells = Array.from({ length: cellCount }, (_, index) => index < offset || index>=offset+dayCount ? 0 : index - offset + 1);
  ...
  return <section className="company-calendar-card"><div className="matrix-demand-head"><div><h3>Grafik zespołu • {scopeLabel}</h3><p>Pełne tygodnie od poniedziałku do niedzieli — tak samo jak w widoku właściciela i lidera. Widzisz wyłącznie dozwolone kategorie, bez cudzych kosztów, dostępności i powodów nieobecności.</p></div></div><div className="company-week-board">{weeks.map((week,weekIndex)=><section className="company-week-row" key={weekIndex}><header><small>TYDZIEŃ {weekIndex+1}</small><strong>{week.filter(Boolean).length?`${month}-${String(week.find(Boolean)).padStart(2,"0")} – ${month}-${String([...week].reverse().find(Boolean)).padStart(2,"0")}`:"Poza miesiącem"}</strong></header><div>{week.map((day,dayIndex)=>{
...
function AvailabilityCalendarDrawer({ workspace, month, locations, close, save, fail, busy, calendarContext, assignments, standby=[], swapAnnouncements=[], embedded=false, onSelectDay }: {
  ...
  locations: MatrixItem[];
  close?: () => void;
  save: (entry: { dates: string[]; kind: "AVAILABLE" | "PREFER_NOT_TO_WORK" | "CANNOT_WORK" | "LEAVE" | "SICKNESS"; allDay: boolean; start?: string; end?: string; preferredLocationId?: string; note: string }) => Promise<boolean>;
  fail: (message: string) => void;
  busy: boolean;
  calendarContext?: WorkforceCalendarContext;
  assignments: PortalAssignment[];
  ...
  const [kind,setKind]=useState<"AVAILABLE"|"PREFER_NOT_TO_WORK"|"CANNOT_WORK"|"LEAVE"|"SICKNESS">("AVAILABLE");
  ...
  const supabase=useMemo(()=>createSupabaseBrowserClient(),[]);
  const [myRequests,setMyRequests]=useState<Array<{id:string;requestType:string;dateFrom:string;dateTo:string;status:string}>>([]);
  const loadMyRequests=useCallback(async()=>{
    if(!supabase)return;
    const result=await supabase.rpc("personal_action_workspace_uat_v1");
    if(!result.error)setMyRequests(((result.data as {myRequests?:typeof myRequests})?.myRequests??[]));
  },[supabase]);
  ...
  const dayRequests=(date:string)=>myRequests.filter(request=>date>=request.dateFrom&&date<=request.dateTo&&["PENDING","APPLIED"].includes(request.status));
  const dayIsProtected=(date:string)=>(entriesByDay.get(date)??[]).some(entry=>!entry.editable||entry.source!=="GRAFIK_PRO")||dayRequests(date).length>0;
  ...
  const submit=async()=>{
    try{
      ...
      if(ok){setSelectedDays([]);setRangeAnchor(null);setNote("");setPreferredLocationId("");await loadMyRequests();}
  ...
  const body=<div className="availability-calendar-content">
      ...
      <section className="availability-calendar-panel">
        ...
        <div className="availability-state-calendar">{Array.from({length:calendarOffset},(_,index)=><i key={`blank-${index}`}/>)}{monthDates.map(date=>{
          ...
          const requestEntries=dayRequests(date);
          const pending=(calendarContext?.pendingReviews.some(review=>review.date===date)??false)||requestEntries.some(request=>request.status==="PENDING");
  ...
  if(embedded)return <section className="employee-combined-calendar"><div className="matrix-demand-head"><div><h3>Mój grafik — {labelMonth(month,timezone)}</h3><p>Grafik i dostępność w jednym kalendarzu: zmiany, rezerwa, urlop, L4 i twarde nieobecności. Kliknij dzień lub zakres i wybierz, co chcesz zgłosić.</p></div></div>{body}</section>;
...
function CoworkerDrawer({ assignment, timezone, dynamic, roleNames, close, requestSwap, loadSwapCandidates, allowSwap, busy, preferredTargetEmployeeId="" }: { assignment: PortalAssignment; timezone: string; dynamic: boolean; roleNames: Record<string, string>; close: () => void; requestSwap: (assignmentId: string, targetEmployeeId: string | null, message: string) => Promise<boolean>; loadSwapCandidates: (assignmentId: string) => Promise<ShiftSwapCandidate[]>; allowSwap: boolean; busy: boolean; preferredTargetEmployeeId?:string }) {
  ...
  const openSwap=async()=>{setSwapOpen(true);setCandidateLoading(true);const rows=await loadSwapCandidates(assignment.id);setCandidates(rows);if(preferredTargetEmployeeId&&rows.some(candidate=>candidate.employeeId===preferredTargetEmployeeId&&candidate.eligible))setTargetEmployeeId(preferredTargetEmployeeId);setCandidateLoading(false);};
  useEffect(()=>{if(preferredTargetEmployeeId)void openSwap();},[]); // eslint-disable-line react-hooks/exhaustive-deps
...
function translateError(message: string) {
  const translations: Record<string, string> = {
    ...
    EVENT_HAS_NO_ACTIVE_ROLE_SHIFTS: "W wybranym zakresie nie ma ani jednego dnia, w którym ta rola występuje na zaznaczonych zmianach. Zmień rolę, lokal, daty albo wybór zmian.",
  };
