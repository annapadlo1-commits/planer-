...
import {
  ...
  CloudDownload, Download, Edit3, FileSpreadsheet, GitBranch, Layers3, Link2, MapPin, Plus,
...
import { buildWorkforceFinanceTemplate } from "@/lib/workforce-finance-export";
...
import {
  ...
  downloadSelectedGoogleDriveWorkbook,
...
export function MatrixV2Editor({
  ...
  async function saveEmployeeProfile(employeeId: string | null, payload: Record<string, unknown>) {
    if (!supabase) return false;
    setBusy(true);
    const result = await supabase.rpc("matrix_v2_employee_save_uat_v4", {
...
function AccessTab({notify,fail}:{notify:(message:string)=>void;fail:(message:string)=>void}){
  ...
  const [googleImportReady,setGoogleImportReady]=useState(false);
  useEffect(()=>{
    const status=googleDriveRedirectStatus();
    if(status==="import_ready_access"){
      clearGoogleDriveRedirectStatus();
      setGoogleImportReady(true);
      return;
    }
    if(status==="ready"){
    ...
    if(redirectMessage){
      const pickerFailure=status==="picker_cancelled"||status==="picker_selection_error";
      setGoogleFallback(!pickerFailure);setGoogleStatusMessage(redirectMessage);clearGoogleDriveRedirectStatus();
      if(pickerFailure)fail(redirectMessage);
    }
  ...
  useEffect(()=>{
    if(!googleImportReady||!directory)return;
    setGoogleImportReady(false);
    void importAccessWorkbookFromGoogleDrive();
  },[directory,googleImportReady]);
  ...
  async function importAccessWorkbookFromGoogleDrive(){
    setSaving(true);setGoogleStatusMessage("");
    try{
      const driveFile=await downloadSelectedGoogleDriveWorkbook();
      if(await inspectAccessWorkbook(driveFile))notify(`Pobrano „${driveFile.name}” z Dysku Google i pokazano podgląd. Nic nie zostało jeszcze zapisane.`);
    }catch(error){
      if(isGoogleSheetsExportError(error)){
        if(error.code==="GOOGLE_AUTH_REQUIRED")setGoogleFallback(true);
        setGoogleStatusMessage(error.message);
      }
      fail(error instanceof Error?error.message:"Nie udało się pobrać pliku z Dysku Google.");
    }finally{setSaving(false);}
  }
  ...
  async function inspectAccessWorkbook(file:File){
    ...
    try{
      ...
      return true;
    }catch(error){setImportPreview(null);fail(error instanceof Error?error.message:"Nie udało się odczytać pliku dostępów.");return false;}
  }
  ...
  return <section className="access-management">
    ...
    <div className="access-bulk-actions"><span><strong>Zbiorcze nadawanie dostępów</strong><small>Przy pierwszym uruchomieniu możesz nadać lub wyłączyć nawet setki funkcji jednym arkuszem.</small></span><button type="button" className="primary-button" disabled={loading||saving||!directory} onClick={()=>void exportAccessWorkbook(true)}><FileSpreadsheet/> {saving?"Przygotowuję…":googleServerReady?"Utwórz arkusz na Dysku Google":"Otwórz w Google Sheets"}</button><button type="button" className="secondary-button" disabled={loading||saving||!directory} onClick={()=>void exportAccessWorkbook()}><Download/> Pobierz plik Excel</button><button type="button" className="secondary-button" disabled={loading||saving||!directory} onClick={()=>beginGoogleDriveImportAuthorization("access")}><CloudDownload/> Importuj z Dysku Google</button><button type="button" className="primary-button" disabled={loading||saving} onClick={()=>accessFileRef.current?.click()}><Upload/> Importuj plik lokalny</button><input ref={accessFileRef} hidden type="file" accept=".xlsx,.xls" onChange={event=>{const file=event.target.files?.[0];if(file)void inspectAccessWorkbook(file);}}/></div>
...
function WorkforceTab({
  ...
  return <div id="configuration-step-employees" className="matrix-v2-tab-content matrix-v2-workforce">
    ...
    <section className={`matrix-v2-card workforce-profile ${employee.active?"":"archived"}`}>
      ...
      <dl><div><dt>Status</dt><dd>{employee.active?"Aktywny":"Archiwalny"}</dd></div><div><dt>Etap</dt><dd>{employee.employmentStage==="PROBATION"?`Okres próbny${employee.probationEnd?` do ${employee.probationEnd}`:""}`:employee.employmentStage==="NOTICE"?"Okres wypowiedzenia":"Stała współpraca"}</dd></div><div><dt>Forma współpracy</dt><dd>{contractTypeLabel(employee.contractType)}</dd></div><div><dt>{flexibleContractor?"Plan ewidencyjny":"Nominał"}</dt><dd>{Math.round(Number(employee.nominalMonthlyMinutes??0)/60)} godz./mies.{flexibleContractor?" • nie blokuje silnika":""}</dd></div><div><dt>{flexibleContractor?"Uzgodniony pułap":"Twardy limit miesięczny"}</dt><dd>{Math.round(Number(employee.maximumMonthlyMinutes??0)/60)} godz.{flexibleContractor?" • informacyjnie":""}</dd></div><div><dt>Zatrudnienie</dt><dd>{employee.employmentStart??"bez daty"}{employee.employmentEnd?` – ${employee.employmentEnd}`:""}</dd></div></dl>
...
async function buildQuickMatrixTemplate(data:MatrixV2Workspace,mode:QuickWorkbookMode,companyBoundaryId:string){
  const XLSX=await import("xlsx");
  const workbook=XLSX.utils.book_new();
  ...
  const meta=XLSX.utils.aoa_to_sheet([["Klucz","Wartość"],["workbookMode",mode],["contractVersion","2"],
    ...(includeCurrent?[["sourceMatrixVersionId",data.matrixVersion.id]]:[]),
...
async function buildMatrixTemplate(data:MatrixV2Workspace,variant:"FULL"|"QUICK",quickMode:QuickWorkbookMode,companyBoundaryId:string){
  if(variant==="QUICK")return buildQuickMatrixTemplate(data,quickMode,companyBoundaryId);
  const XLSX=await import("xlsx");
  const workbook=XLSX.utils.book_new();
  ...
  const instructions=XLSX.utils.aoa_to_sheet([
    ...
    ["Kategorie grafików","Każdą rolę przypisz do jednej kategorii. Generator uruchamia się dla całej kategorii i planuje razem wszystkie jej role oraz obowiązki."],
    ["Role podstawowe i rezerwowe","Rola podstawowa to zwykła praca osoby. W polu Role rezerwowe wpisz np. HOST:1, RUNNER:2; silnik użyje ich dopiero, gdy nie da się utrzymać pełnej obsady rolami standardowymi."],
    ["Pula ad-hoc","Osoby z arkusza Pula ad-hoc nie trafiają do zwykłego grafiku. System pokaże je dopiero jako propozycję naprawy braków."],
  ...
  const employeeRows=data.employees.filter(employee=>employee.active).map(employee=>{
    ...
    const backupRoles=(data.employeeRoles??[]).filter(item=>item.employee_id===employee.id&&item.active&&!item.is_primary&&item.assignment_mode==="BACKUP")
      .sort((left,right)=>(left.backup_priority??100)-(right.backup_priority??100))
      .map(item=>`${data.roles.find(roleItem=>roleItem.id===item.role_id)?.code??""}:${item.backup_priority??100}`).filter(value=>!value.startsWith(":"));
    ...
    return [employee.employeeNo,employee.active?"TAK":"NIE",employee.firstName,employee.lastName,employee.email??"",role?.code??"",backupRoles.join(", "),employee.employmentStage??"REGULAR",employee.probationEnd??"",locationCodes.join(", "),baseLocationCode,...locationGrantCells,employee.employmentStart??"",employee.employmentEnd??"",Number(employee.nominalMonthlyMinutes??0)/60,Number(employee.maximumMonthlyMinutes??0)/60,Number(employee.maximumWeeklyMinutes??0)/60,employee.maximumConsecutiveDays,employee.minimumRestMinutes===null||employee.minimumRestMinutes===undefined?"":employee.minimumRestMinutes/60,employee.noWeekends?"TAK":"NIE",activeRate?activeRate.base_rate_minor/100:"",employee.contractType??activeRate?.contract_type??"INNE",employee.workTimePolicy??"CONTRACT_DEFAULT",employee.overtimePolicy??"NEVER",...activeDutyCodes.map(code=>duties.has(code)?"TAK":"")];
  });
  ...
  XLSX.utils.book_append_sheet(workbook,XLSX.utils.aoa_to_sheet([
    ["Klucz","Wartość"],
    ["workbookMode","CURRENT_CONFIG_EXPORT"],
    ["contractVersion","2"],
    ["sourceMatrixVersionId",data.matrixVersion.id],
    ["companyBoundaryId",companyBoundaryId],
  ]),"_META");
  const raw=XLSX.write(workbook,{type:"array",bookType:"xlsx"});
  const {polishMatrixWorkbook}=await import("@/lib/excel-workbook-polish");
...
function MatrixExcelImport({data,busy,setBusy,close,reload,notify,fail}:{data:MatrixV2Workspace;busy:boolean;setBusy:(value:boolean)=>void;close:()=>void;reload:()=>Promise<void>;notify:(message:string)=>void;fail:(message:string)=>void}){
  ...
  const importScopeKey=`grafik-pro:matrix-v2:${data.matrixVersion.id}:import-scope`;
  const restored=useMemo(()=>readStoredMatrixImport(data.matrixVersion.id),[data.matrixVersion.id]);
  const [scope,setScope]=useState<MatrixImportScope>(()=>{
    if(restored?.scope)return restored.scope;
    if(typeof window==="undefined")return "TEAM";
    const saved=window.sessionStorage.getItem(importScopeKey);
    return saved==="FINANCE"||saved==="CONFIGURATION"?saved:"TEAM";
  });
  ...
  useEffect(()=>{
    const status=googleDriveRedirectStatus();
    if(status==="import_ready_matrix"){
      clearGoogleDriveRedirectStatus();
      void importMatrixWorkbookFromGoogleDrive();
      return;
    }
    if(status==="ready"){
      setGoogleServerReady(true);setGoogleFallback(false);setGoogleStatusMessage("");
  ...
  useEffect(()=>{window.sessionStorage.setItem(importScopeKey,scope);},[importScopeKey,scope]);
  ...
  function beginMatrixGoogleDriveImport(){
    window.sessionStorage.setItem(importScopeKey,scope);
    beginGoogleDriveImportAuthorization("matrix");
  }
  async function exportTemplate(toGoogle=false,quickMode:QuickWorkbookMode="CURRENT_CONFIG_EXPORT"){
    let googleWindow:Window|null=null;
    setBusy(true);setLocalError("");
    try{
      if(!supabase)throw new Error("Nie udało się potwierdzić firmy dla tego projektu. Odśwież stronę i spróbuj ponownie w Ustawieniach → Konfiguracja firmy.");
      ...
      const boundaryResult=await supabase.rpc("matrix_v2_company_boundary_uat_v1");
      if(boundaryResult.error)throw new Error(matrixV2ErrorMessage(boundaryResult.error.message));
      const companyBoundaryId=String(boundaryResult.data??"").trim();
      if(!/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/iu.test(companyBoundaryId)){
        throw new Error("Nie udało się potwierdzić firmy dla tego projektu. Odśwież stronę i pobierz nowy plik w Ustawieniach → Konfiguracja firmy.");
      }
      const artifact=scope==="FINANCE"
        ?await buildWorkforceFinanceTemplate(data,companyBoundaryId)
        :await buildMatrixTemplate(data,scope==="TEAM"?"QUICK":"FULL",quickMode,companyBoundaryId);
      if(toGoogle){
        if(googleServerReady){
          showGoogleSheetsInTargetWindow(googleWindow!,await uploadWorkbookToGoogleSheetsViaServer(artifact.bytes,artifact.fileName));
          return;
  ...
  async function inspect(selectedFile=file){
    if(!selectedFile||!supabase)return false;
    setBusy(true);setLocalError("");setLocalIssues([]);setPreview(null);
    try{
      if(scope==="FINANCE"){
        const parsed=await readWorkforceFinanceWorkbook(selectedFile);
      ...
      const [configuration,finance]=await Promise.all([readMatrixWorkbook(selectedFile),readWorkforceFinanceWorkbook(selectedFile)]);
      ...
      return true;
    }catch(error){if(error instanceof MatrixWorkbookValidationError)setLocalIssues(error.issues);setLocalError(error instanceof Error?error.message:"Nie udało się odczytać pliku Excel.");return false;}
    finally{setBusy(false);}
  }
  async function importMatrixWorkbookFromGoogleDrive(){
    setBusy(true);setLocalError("");setLocalIssues([]);setPreview(null);
    try{
      const driveFile=await downloadSelectedGoogleDriveWorkbook();
      clearPersistedImport();setFile(driveFile);setPayload(null);
      if(await inspect(driveFile))notify(`Pobrano „${driveFile.name}” z Dysku Google i pokazano podgląd. Nic nie zostało jeszcze zapisane.`);
    }catch(error){
      if(isGoogleSheetsExportError(error)){
        if(error.code==="GOOGLE_AUTH_REQUIRED")setGoogleFallback(true);
        setGoogleStatusMessage(error.message);
      }
      setLocalError(error instanceof Error?error.message:"Nie udało się pobrać pliku z Dysku Google.");
    }finally{setBusy(false);}
  }
  ...
  return <><button className="drawer-scrim top" onClick={close}/><aside className="drawer matrix-v2-drawer top">
    ...
    <div className="drawer-content">
      ...
      <section className="matrix-import-preview"><h3>IMPORTUJ PLIK</h3><p>Najpierw sprawdzimy cały plik i pokażemy podgląd. Żaden rekord nie zostanie zapisany na tym etapie.</p><button className="secondary-button full" type="button" disabled={busy} onClick={beginMatrixGoogleDriveImport}><CloudDownload/> Importuj z Dysku Google</button><small>Wybierz dokładnie jeden Arkusz Google albo XLSX. System pobierze go do podglądu i nie zapisze danych automatycznie.</small><label>Lub wybierz plik lokalny .xlsx albo .xls<input type="file" accept=".xlsx,.xls" onChange={event=>{clearPersistedImport();setFile(event.target.files?.[0]??null);setPayload(null);setPreview(null);setLocalError("");}}/></label></section>
...
function EmployeeProfileDrawer({employee,data,month,busy,close,save}:{employee:MatrixV2Employee|null;data:MatrixV2Workspace;month:string;busy:boolean;close:()=>void;save:(employeeId:string|null,payload:Record<string,unknown>)=>Promise<boolean>}) {
  ...
  async function submit(form:HTMLFormElement){
    try{
      ...
      await save(employee?.id??null,{
        ...
        employmentStage:formText(form,"employmentStage")||"REGULAR",
        probationEnd:formText(form,"probationEnd")||null,
        ...
        overtimePolicy:formText(form,"overtimePolicy")||"NEVER",
  ...
  return <><button className="drawer-scrim top" onClick={close}/><aside className="drawer matrix-v2-drawer top">
    ...
    <form className="drawer-content" onSubmit={event=>{event.preventDefault();void submit(event.currentTarget);}}>
      ...
      <div className="form-row"><label>Etap zatrudnienia<select name="employmentStage" defaultValue={employee?.employmentStage??"REGULAR"}><option value="REGULAR">Stała współpraca</option><option value="PROBATION">Okres próbny</option><option value="NOTICE">Okres wypowiedzenia</option></select><small>Etap jest informacją operacyjną; nie zmienia samodzielnie uprawnień ani zasad grafiku.</small></label><label>Koniec okresu próbnego<input name="probationEnd" type="date" defaultValue={employee?.probationEnd??""}/><small>Wypełnij wyłącznie dla osoby na okresie próbnym.</small></label></div>
      ...
      <label>Zgoda na nadgodziny<select name="overtimePolicy" defaultValue={employee?.overtimePolicy??"NEVER"}><option value="NEVER">NIE — generator nie może tworzyć nadgodzin</option><option value="APPROVAL_REQUIRED">TYLKO PO ZATWIERDZENIU — pokaż propozycję liderowi</option><option value="ALLOWED">TAK — generator może użyć po wykorzystaniu zwykłego wymiaru</option></select><small>Wszystkie trzy warianty najpierw układają grafik bez nadgodzin. Dla „tylko po zatwierdzeniu” automatyczny grafik pozostawi brak do świadomej decyzji lidera.</small></label>
...
function DrawerFields({kind,item,data,month,operation,setOperation,payMethod,setPayMethod,scenarioStrategyId,setScenarioStrategyId}:{kind: MatrixV2SaveKind; item?: Record<string, unknown>; data: MatrixV2Workspace; month: string; operation: string; setOperation: (value: string) => void; payMethod: string; setPayMethod: (value: string) => void; scenarioStrategyId: string; setScenarioStrategyId: (value: string) => void}) {
  ...
  if (["ROLE","LOCATION","DUTY","STRATEGY"].includes(kind)) return <>
    ...
    {kind === "ROLE" && <label>Kategoria grafiku<select name="categoryId" required defaultValue={String(item?.category_id ?? "")}><option value="" disabled>Wybierz kategorię</option>{(data.roleCategories??[]).filter(category=>category.active||category.id===item?.category_id).map(category=><option value={category.id} key={category.id}>{category.name}</option>)}</select><small>Wszystkie role w tej kategorii są optymalizowane razem w jednym grafiku.</small></label>}
