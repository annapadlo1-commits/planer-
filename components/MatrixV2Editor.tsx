...
async function buildMatrixTemplate(data:MatrixV2Workspace,variant:"FULL"|"QUICK"="FULL"){
  ...
  add("Pula ad-hoc",["Imię i nazwisko","E-mail","Telefon","Kod roli","Rodzaj współpracy","Stawka godzinowa","Waluta","Dostępny od","Dostępny do","Notatki","Aktywna"],(data.adHocWorkers??[]).map(item=>[
    item.display_name,item.email??"",item.phone??"",data.roles.find(roleItem=>roleItem.id===item.role_id)?.code??"",item.contract_type,item.base_rate_minor===null||item.base_rate_minor===undefined?"":item.base_rate_minor/100,item.currency,item.available_from??"",item.available_to??"",item.notes??"",item.active?"TAK":"NIE",
  ]));
