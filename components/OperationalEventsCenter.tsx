...
export function OperationalEventsCenter({month,notify,fail}:{month:string;notify:(message:string)=>void;fail:(message:string)=>void}){
  ...
  const roles=workspace?.roles.filter(role=>!form.categoryIds.length||!!role.categoryId&&form.categoryIds.includes(role.categoryId))??[];
  const toggleScope=(field:"categoryIds"|"roleIds",id:string)=>setForm(current=>{
    ...
    if(field==="categoryIds")return {
      ...
      roleIds:current.roleIds.filter(roleId=>!next.length||!!workspace?.roles.find(role=>role.id===roleId&&(!role.categoryId||next.includes(role.categoryId)))),
    };
