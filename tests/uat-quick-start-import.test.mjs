...
test("quick workbook documents automatic identifiers and physically excludes advanced sheets", () => {
  ...
  assert.match(quickBuilder, /szafunek-pusty-szablon-konfiguracji-v2\.xlsx/);
  assert.match(quickBuilder, /szafunek-obecna-konfiguracja-v/);
  assert.match(editor, /EMPTY_TEMPLATE/);
  assert.match(quickBuilder, /CURRENT_CONFIG_EXPORT/);
