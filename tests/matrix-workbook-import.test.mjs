...
test("a freshly downloaded quick-start workbook remains self-importable after an empty UAT reset",async()=>{
  ...
  assert.equal(parsed.settings.strategySemanticsVersion,"B4F170_V1");
  ...
  assert.equal(parsed.strategyObjectives.length,24);
  assert.equal(parsed.strategyObjectives.some(item=>item.metricCode==="HOME_LOCATION_VIOLATIONS"),false);
