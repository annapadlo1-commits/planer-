...
type NavKey = "centrum"|"generator"|"zespoly"|"matrix"|"grafik"|"kalendarz"|"pracownicy"|"alerty"|"budzet";
...
const nav = [
  ...
  ["pracownicy","Widok pracownika",Users],["alerty","Braki i alerty",AlertTriangle],
  ["budzet","Budżet i koszty",CircleDollarSign],
] as const;
...
export default function GrafikPro() {
  ...
  return <main className="app-shell">
    <aside className="sidebar">
      <div className="brand"><span>GP</span><div><strong>GRAFIK PRO</strong><small>ALPHA 6 • TEAM MATRIX</small></div></div>
      ...
      <div className="content">
        {active==="centrum"&&<>
          ...
          <section className="live-overview">
            ...
            <div className="quick-actions"><button onClick={()=>setActive("grafik")}>Otwórz grafik <ChevronRight/></button><button onClick={()=>setActive("pracownicy")}>Obciążenie pracowników <ChevronRight/></button><button onClick={()=>setActive("alerty")}>Rozwiąż alerty <ChevronRight/></button>{data.plan.status!=="PUBLISHED"&&<button className="publish" onClick={()=>void publish()}>Opublikuj wariant <Check/></button>}</div>
          </section>}
        </>}
        ...
        {active==="zespoly"&&<RolePlanningView matrix={matrix} busy={busy} onCreate={async(roleId,roleName)=>{if(!supabase)return;setBusy(true);const result=await supabase.rpc("create_role_plan_section",{p_month:MONTH,p_role_id:roleId,p_name:`Grafik ${roleName} • lipiec 2026`,p_scenario:"BASE",p_mode:"BALANCED",p_staffing:"OPTIMAL"});setBusy(false);if(result.error)setError(result.error.message);else{notify("Utworzono nowy wariant zespołu");await load();}}} onTransition={async(id,status)=>{if(!supabase)return;setBusy(true);const result=await supabase.rpc("transition_role_plan",{p_section_id:id,p_status:status});setBusy(false);if(result.error)setError(result.error.message);else{notify("Zmieniono etap grafiku zespołu");await load();}}}/>} 
        {active==="matrix"&&<MatrixView matrix={matrix}/>} 
        {active==="kalendarz"&&<MonthView data={data} onDay={(d)=>{setDay(d);setActive("grafik");}} onEvent={()=>setModal("event")}/>}
        {active==="pracownicy"&&<EmployeeView employees={employees} selected={selectedEmployee} onSelect={(id)=>{setSelectedEmployee(id);setActive("grafik");}}/>}
