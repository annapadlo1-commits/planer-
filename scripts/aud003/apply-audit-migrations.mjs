...
const ledger=JSON.parse(await readFile('supabase/migrations/ledger.manifest.json','utf8'));
assert.equal(ledger.projectRef,'nhthrtpkfpmufmrmdyjg');
assert.equal(ledger.entries[0]?.file,'20260903090000_aud003_canonical_uat_baseline.sql');
const files=ledger.entries.slice(1);
...
for(const entry of files) {
 ...
 assert.equal(createHash('sha256').update(canonical).digest('hex'),entry.canonicalSha256,'AUDIT_MIGRATION_CHANGED');
 ...
 results.push({file:entry.file,canonicalSha256:entry.canonicalSha256,transaction:wrapped?'original explicit BEGIN/COMMIT + ON_ERROR_STOP':'psql --single-transaction + ON_ERROR_STOP',exitCode:r.status,durationMs:Math.round(performance.now()-start),stdout:r.stdout,stderr:r.stderr});
