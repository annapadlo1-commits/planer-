...
begin
  ...
  where mv.settings->>'strategySemanticsVersion'='B4F169_V1'
    and mv.schema_version>=2
  order by mv.version desc
  limit 1;

  if v_active.settings->>'strategySemanticsVersion'<>'B4F169_V1' then
    raise exception 'B4F169_HISTORICAL_MATRIX_SEMANTICS_MISMATCH';
  end if;
  if v_active.settings->'fairnessQualityGate'<>jsonb_build_object(
    ...
    raise exception 'B4F169_HISTORICAL_MATRIX_GATE_MISMATCH';
  end if;
  ...
  select pg_get_functiondef(
    'solver_private.build_snapshot_payload_before_b4f170(uuid,date,uuid,uuid,text,uuid)'::regprocedure
  ...
  if has_function_privilege(
    ...
    raise exception 'B4F169_V21_SNAPSHOT_BUILDER_EXPOSED';
  end if;
end;
$$;
