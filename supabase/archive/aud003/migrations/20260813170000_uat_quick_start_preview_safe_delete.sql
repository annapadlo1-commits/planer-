-- UAT only: the import preview executes the ordinary import inside a rolled-back
-- subtransaction. Keep the full-table ad-hoc replacement explicit so the
-- database safe-mutation guard accepts both preview and apply paths.
