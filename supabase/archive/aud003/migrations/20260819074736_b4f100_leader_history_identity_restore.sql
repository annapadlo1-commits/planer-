-- B4F-100: plan_issues_v2.id is GENERATED ALWAYS. Restoring an exact history
-- snapshot must explicitly opt into the historical identity values.
...
revoke all on function public.optimizer_leader_history_move_uat_v1(uuid,text),
  public.optimizer_leader_checkpoint_restore_uat_v1(uuid,bigint,text)
  from public,anon,authenticated;
grant execute on function public.optimizer_leader_history_move_uat_v1(uuid,text),
  public.optimizer_leader_checkpoint_restore_uat_v1(uuid,bigint,text)
  to authenticated;

comment on function public.optimizer_leader_history_move_uat_v1(uuid,text)
is 'B4F-100: restores exact issue identities for undo/redo using OVERRIDING SYSTEM VALUE.';
comment on function public.optimizer_leader_checkpoint_restore_uat_v1(uuid,bigint,text)
is 'B4F-100: restores exact issue identities for named checkpoints using OVERRIDING SYSTEM VALUE.';
