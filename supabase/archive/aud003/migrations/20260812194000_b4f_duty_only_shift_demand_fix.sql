-- B4F-23: a duty-only staffing row (for example Host or Runner) already
-- expresses the complete demand for that shift. A role-duty link describes
-- competency; it creates recurring shift demand only when it is explicitly
-- marked as a shift obligation for the matching period.
...
comment on function solver_private.resolved_demand_v2(date,uuid,uuid,uuid) is
  'Resolves monthly staffing demand. Only period-scoped shift obligations consume generic staffing; duty-only staffing rows remain independent demand slots.';
