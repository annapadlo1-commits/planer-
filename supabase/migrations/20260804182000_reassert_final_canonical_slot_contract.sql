-- Reassert the canonical empty-duty slot identifier after the later snapshot
-- extensions for stand-by and events.  Both extensions replaced the snapshot
-- builder and accidentally restored `||` for an empty duty array, while the
-- worker correctly generates `|-|`.

alter function solver_private.build_snapshot_payload_v2(
  uuid,date,uuid,uuid,text,uuid
) rename to build_snapshot_payload_before_final_slot_contract_v2;
...
revoke all on function solver_private.build_snapshot_payload_before_final_slot_contract_v2(
  uuid,date,uuid,uuid,text,uuid
) from public,anon,authenticated;
revoke all on function solver_private.build_snapshot_payload_v2(
  uuid,date,uuid,uuid,text,uuid
) from public,anon,authenticated;
grant execute on function solver_private.build_snapshot_payload_before_final_slot_contract_v2(
  uuid,date,uuid,uuid,text,uuid
) to service_role;
