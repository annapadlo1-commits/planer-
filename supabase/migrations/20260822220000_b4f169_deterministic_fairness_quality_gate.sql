-- B4F-169 UAT: the same business snapshot must not receive a new random seed
-- merely because it has a new run id. Publish a new Matrix contract with the
-- owner-approved 70% minimum / 30 p.p. spread gate and three deterministic
-- attempts. Historical Matrix versions and stored snapshots remain immutable.

create temp table b4f169_prior_active on commit drop as
...
do $$
begin
  if (select count(*) from b4f169_prior_active)<>1 then
    raise exception 'B4F169_ACTIVE_MATRIX_REQUIRED';
  end if;
  if exists(
    select 1 from public.matrix_versions mv
    where mv.status='DRAFT' and mv.schema_version>=2
  ) then
    raise exception 'B4F169_EXISTING_DRAFT_REQUIRES_OWNER_DECISION';
  end if;
end;
$$;

create or replace function solver_private.validate_strategy_semantics_b4f169(
...
-- The existing trigger keeps this stable function name.
...
create or replace function solver_private.apply_strategy_semantics_b4f169(
  p_matrix_version_id uuid
)
returns void
language plpgsql
...
alter function solver_private.matrix_v2_seed_required_defaults_uat_v1(uuid)
  rename to matrix_v2_seed_required_defaults_before_b4f169;
...
-- Wrap the complete, already-audited snapshot builder after it has added the
-- baseline, locked and external assignments. Only runId and generated seeds
-- are excluded from the seed source; every business input remains included.
alter function solver_private.build_snapshot_payload_v2(
  uuid,date,uuid,uuid,text,uuid
) rename to build_snapshot_payload_before_b4f169;
...
revoke all on function
  solver_private.validate_strategy_semantics_b4f169(uuid),
  solver_private.validate_strategy_semantics_b4f165(uuid),
  solver_private.apply_strategy_semantics_b4f169(uuid),
  solver_private.matrix_v2_seed_required_defaults_before_b4f169(uuid),
...
begin
  ...
  limit 1;
  if v_owner is null then raise exception 'B4F169_OWNER_REQUIRED'; end if;
  perform set_config('request.jwt.claim.sub',v_owner::text,true);
  v_draft:=public.matrix_v2_create_draft(
    'B4F-169 — deterministyczna bramka jakości fairness 30 p.p.'
  );
  perform solver_private.apply_strategy_semantics_b4f169(v_draft);
...
declare
  ...
  v_prior b4f169_prior_active%rowtype;
begin
  select * into v_prior from b4f169_prior_active;
  ...
  if v_active.id is null
    ...
    raise exception 'B4F169_NEW_ACTIVE_MATRIX_NOT_PUBLISHED';
  end if;
  ...
  if v_active.settings->'fairnessQualityGate'<>jsonb_build_object(
    ...
    raise exception 'B4F169_QUALITY_GATE_MISMATCH';
  end if;
  if (select mv.content_hash from public.matrix_versions mv where mv.id=v_prior.id)
    ...
    raise exception 'B4F169_HISTORICAL_MATRIX_CONTENT_CHANGED';
  end if;

  insert into public.audit_log(actor_id,entity_type,entity_id,action,new_data)
  values(auth.uid(),'matrix_v2',v_active.id::text,
    'B4F169_FAIRNESS_QUALITY_GATE_PUBLISHED',jsonb_build_object(
...
-- Keep the per-run database stamp aligned with this migration.
alter function public.solver_save_variant_v2(uuid,uuid,uuid,jsonb,text)
  rename to solver_save_variant_before_b4f169;
...
revoke all on function public.solver_save_variant_before_b4f169(
